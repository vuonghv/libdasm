...
def main():
    ...
    while offset < data_len:
        ...
        if not instruction or (instruction.length+offset) > data_len:
            if bytes:
                str += '%.2x  ' % int_(data[offset]) + ' '*(bytes-1)*2
            if format == FORMAT_INTEL:
                str += 'db 0x%.2x' % int_(data[offset])
            else:
                str += '.byte 0x%.2x' % int_(data[offset])
            print(str)
            offset += 1
            continue
